...
public class PartitionArrayForPivot {
    public static int[] pivotArray(int[] nums, int pivot) {
        ...
        for(int i=0; i<lowerElements.size(); i++)
            partitionedArray[pivotIndex++] = lowerElements.get(i);

        while(pivotCount != 0){
            partitionedArray[pivotIndex++] = pivot;
            pivotCount--;
        }

        for(int i=0; i<greaterElements.size(); i++)
            partitionedArray[pivotIndex++] = greaterElements.get(i);
